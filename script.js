// --- Tutor Dashboard Script (Renamed, Enhanced Skills, Mobile Nav for Tutor & Student View) ---
...
for (let i = 0; i < 10; i++) { 
    ...
    const student = { id: `student${String(i+1).padStart(3,'0')}`, name: `${firstNames[i]} ${lastNames[i]}`, targetScore: 1300 + Math.floor(Math.random() * 6) * 50, cbPracticeTests: [], eocQuizzes: { reading: [], writing: [], math: [] }, khanAcademyPractice: { reading: [], writing: [], math: [] }, skills: { reading: [], writing: [], math: [] }, activityMetrics: { avgTimePerModuleCB: 100 + Math.floor(Math.random() * 50), eocAttempts: 10 + Math.floor(Math.random() * activityScore / 3), khanActivitiesCompleted: 5 + Math.floor(Math.random() * activityScore / 5), pagesViewed: 80 + Math.floor(Math.random() * activityScore * 1.5), latestTotalScore: latestTotalScore, previousTotalScore: previousTotalScore, activityScore: activityScore }, scoreHistory: [previousTotalScore - Math.floor(Math.random()*50), previousTotalScore, latestTotalScore] };
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    tutorTabs = document.querySelectorAll('nav .main-nav-button'); // More specific selector for tutor main nav
    tutorTabContents = document.querySelectorAll('main > .tutor-tab-content');
    ...
    } else { console.warn("Tutor hamburger button or mobile menu not found. Mobile nav for tutor might not work."); }
    ...
    if (replicatedStudentDashboardDiv) {
        ...
        function switchStudentDeepDiveTab(studentTabElement) {
            ...
            const firstSubTab = replicatedStudentDashboardDiv.querySelector(`#${targetContentId} .sub-tab-button`);
    ...
    } else { console.error("replicatedStudentDashboardDiv not found, student deep dive tabs not fully initialized."); }
...
function initializeStudentOverviewCharts(student) { 
    ...
    if (studentOverallSkillCanvas) { 
        ...
        let perfLabels = ['Reading', 'Writing & Language', 'Math']; let perfStudent = [0,0,0]; let perfClass = [0,0,0];
        const rSkills = student.skills.reading.flatMap(cat => cat.skills); const wSkills = student.skills.writing.flatMap(cat => cat.skills); const mSkills = student.skills.math.flatMap(cat => cat.skills);
        const readingAvg = rSkills.reduce((acc, s) => acc + (s.attempted ? s.score : 0), 0) / (rSkills.filter(sk=>sk.attempted).length || 1);
        const writingAvg = wSkills.reduce((acc, s) => acc + (s.attempted ? s.score : 0), 0) / (wSkills.filter(sk=>sk.attempted).length || 1);
        const mathAvg = mSkills.reduce((acc, s) => acc + (s.attempted ? s.score : 0), 0) / (mSkills.filter(sk=>sk.attempted).length || 1);
        perfStudent = [readingAvg || 0, writingAvg || 0, mathAvg || 0].map(s => Math.round(s));
        perfClass = [ Math.round(Object.keys(classAverages.skills).filter(k=>k.startsWith('reading')).reduce((a,k)=>a+classAverages.skills[k],0) / (Object.keys(classAverages.skills).filter(k=>k.startsWith('reading')).length||1)), Math.round(Object.keys(classAverages.skills).filter(k=>k.startsWith('writing')).reduce((a,k)=>a+classAverages.skills[k],0) / (Object.keys(classAverages.skills).filter(k=>k.startsWith('writing')).length||1)), Math.round(Object.keys(classAverages.skills).filter(k=>k.startsWith('math')).reduce((a,k)=>a+classAverages.skills[k],0) / (Object.keys(classAverages.skills).filter(k=>k.startsWith('math')).length||1)) ];
        studentOverallSkillChartInstance = new Chart(studentOverallSkillCanvas.getContext('2d'), { type: 'bar', data: { labels: perfLabels, datasets: [ { label: 'Student Accuracy', data: perfStudent, backgroundColor: CHART_PRIMARY_BG_BAR}, { label: 'Class Average', data: perfClass, backgroundColor: CHART_SECONDARY_COLOR + 'AA' } ] }, options: { ...chartOptions, scales: { y: { beginAtZero: true, max: 100 }}} }); 
    } else { console.error("studentOverallSkillChart canvas not found for student:", student.name); }
}

function loadClassOverviewData() { /* ... Same as last version, with updated text labels ... */ 
...
function loadStudentPerformanceTiers() { /* ... Same as last version, with renaming applied ... */ 
...
function loadStudentDashboard(studentId) { /* ... Same as last version, with renaming, date formatting, and new skills display ... */ 
...
function openModal(title, contentDetails) { 
    ...
    if(modalDonutChartInstance){ console.log("[openModal] Destroying old donut instance"); modalDonutChartInstance.destroy(); modalDonutChartInstance = null; }
    if(modalLineChartInstance){ console.log("[openModal] Destroying old line instance"); modalLineChartInstance.destroy(); modalLineChartInstance = null; }
    ...
    if (donutCanvas) {
        ...
        if(donutCtx) {
            try {
                ...
                console.log("[openModal] Donut chart instance CREATED:", modalDonutChartInstance);
    ...
    if(modal) modal.style.display="block"; else console.error("Modal element not found in openModal for display:block");
}
function closeModal() { 
...
console.log("Tutor Dashboard script.js: Execution finished.");
